...
def main():
    ...
    p0.append(0.0007)  # Global T2star guess.
    ...
    for fit in all_fits:
        if fit["fit_success"]:
            ...
            results_list.append(
                {"Pulse Time (us)": fit["pulse_time"], "A": A_val, "A_err": A_err_val}
            )
    ...
    p0_sin = [results_df["A"].max(), 1e-3, np.pi / 2]
    popt_sin, pcov_sin = curve_fit(
        abs_sinusoid,
        results_df["Pulse Time (us)"],
        results_df["A"],
        sigma=results_df["A_err_profiled"],
        absolute_sigma=True,
        p0=p0_sin,
    )
    x_fit = np.linspace(
    ...
    plt.errorbar(
        results_df["Pulse Time (us)"],
        results_df["A"],
        yerr=results_df["A_err_profiled"],
        fmt="o",
        capsize=4,
