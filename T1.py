...
# ------------------- USER SETTINGS -------------------
DATA_FOLDER = "T1_data"  # folder where the CSV files are located
...
CROPPING_WINDOWS = {
    ...
    20: (0.012, 0.014),
    30: (0.014, 0.016),
    # add other tau values as needed...
}
# ------------------- END USER SETTINGS -------------------
...
def plot_and_fit(results_df, individual, fit_filename="t1_fit.png"):
    ...
    plt.plot(
        ...
        amp_fit,
        "b--",
        label=f"Exponential Fit\n(A={popt[0]:.2f}, T1={popt[1]:.2f} ms, C={popt[2]:.2f})",
    )
    plt.xlabel("Tau (ms)")
    plt.ylabel("Echo Amplitude")
    plt.title("T1 Measurement via Echo Amplitude vs. Tau")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    chi_sq = np.sum((residuals) ** 2 / (np.var(all_amp) + 1e-6))
